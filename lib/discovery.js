  ...
  MsSQL.prototype.buildQueryTables = function(options) {
    ...
    if (options.all && !owner) {
      sqlTables = this.paginateSQL('SELECT \'table\' AS "type", table_name AS "name", table_schema AS "owner"' +
        ' FROM INFORMATION_SCHEMA.tables WHERE TABLE_TYPE=\' BASE TABLE.\'', 'table_schema, table_name', options);
    } else if (owner) {
      sqlTables = this.paginateSQL('SELECT \'table\' AS "type", table_name AS "name", table_schema AS "owner"' +
        ' FROM INFORMATION_SCHEMA.tables WHERE  TABLE_TYPE=\' BASE TABLE.\' AND table_schema=\'' + owner + '\'', 'table_schema, table_name', options);
    } else {
      sqlTables = this.paginateSQL('SELECT \'table\' AS "type", table_name AS "name",' +
        ' table_schema AS "owner" FROM INFORMATION_SCHEMA.tables WHERE  TABLE_TYPE=\' BASE TABLE.\' AND table_schema=schema_name()',
        'table_name', options);
    }
    return sqlTables;
  };
